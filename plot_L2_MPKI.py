...
data = np.genfromtxt('/home/rashid/extract_data/L2_TLB_MPKI.csv',delimiter=',',skip_header=0,usecols=(0,1))
...
program_names = ["bc", "bfs", "cc", "cc_sv", "sssp", "pr","pr_spmv", "tc", "crono_apsp", "crono_bc", "crono_cc", "crono_pagerank", "crono_sssp", "crono_tc", "crono_tsp", "crono_XSBench", "crono_XSBench_hash" , "crono_XSBench_nuclide", "crono_gups"]

program_names = ["bc", "bfs", "cc", "cc_sv", "sssp", "pr","pr_spmv", "tc", "apsp", "bc", "cc", "pagerank", "sssp", "tc", "tsp", "XSBench", "XSBench_hash" , "XSBench_nuclide", "gups"]
...
ax.set_ylabel('L2 TLB MPKI', fontproperties=prop, fontsize=12)
...
section_positions = [7.5, 14.5,17.5,18.5]  # Example positions where you want to mark the sections
...
for bar, value in zip(bars, baseline):
    ...
    ax.text(bar.get_x() + bar.get_width() / 2, height + 0.2, f'{value:.2f}', ha='center', va='bottom', fontsize=9, color='black')

section_positions = [1.5, 31.5]  # Example positions where you want to mark the sections
#section_labels = ['ASCYLIB', 'SPEC17', 'CRONO']  # Example labels for the sections
...
section_positions = [5,11,16.8,18.4]  # Example positions where you want to mark the sections
section_labels = ['GAPBS', 'CRONO', "XSBench", "GUPS"]  # Example labels for the sections


for position, label in zip(section_positions, section_labels):
    ax.axvline(x=position, color='white', linestyle='', linewidth=4)  # Mark the section with a vertical line
    ax.text(position,4, label, transform=ax.transData, ha='right', va='top', fontsize=12)  # Add the section label
...
ax.axhline(y=1, color='grey', linestyle='--',alpha=0.4)
ax.axhline(y=2, color='grey', linestyle='--',alpha=0.4)
ax.axhline(y=3, color='grey', linestyle='--',alpha=0.4)
ax.axhline(y=4, color='grey', linestyle='--',alpha=0.4)
...
plt.tight_layout()
plt.savefig('L2_TLB_MPKI.pdf',format='pdf', dpi=300)
plt.show()
