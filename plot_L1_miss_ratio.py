...
data = np.genfromtxt('/home/rashid/extract_data/TLB_L1_miss_ratio.csv',delimiter=',',skip_header=0,usecols=(0,1))
...
ax.set_ylabel('L1 TLB miss ratio', fontproperties=prop, fontsize=13)
...
for position, label in zip(section_positions, section_labels):
    ...
    ax.text(position,35, label, transform=ax.transData, ha='right', va='top', fontsize=12)  # Add the section label
...
plt.tight_layout()
plt.savefig('L1_TLB_miss_ratio.pdf',format='pdf', dpi=300, bbox_inches='tight')
plt.show()
