...
for directory_name in directory_names:
    ...
    #txt_file_path_rxt = os.path.join(dir_path_rxt, 'stats.txt')
    if os.path.exists(txt_file_path_rxt):
        with open(txt_file_path_rxt, 'r') as file_vanilla:
            numbers = []
            number_vanilla = 0
            number_vanilla_access = 0
            for line_vanilla in file_vanilla:
                match_vanilla = re.search(r"system\.cpu\.lsq0\.total_finish\s+(\d+)\s*#", line_vanilla)
                match_vanilla_access = re.search(r"system\.cpu\.lsq0\.total_address_done\s+(\d+)\s*#", line_vanilla)
                if match_vanilla_access:
                    number_vanilla_access = float(match_vanilla_access.group(1))
                    numbers.append((directory_name, number_vanilla_access))
                    print(str(number_vanilla_access))
                if match_vanilla:
                    number_vanilla = float(match_vanilla.group(1))
                    print(str(number_vanilla))
                    result.append((directory_name, (number_vanilla_access / number_vanilla)*100))
                    numbers_gmean_vanilla.append(float(number_vanilla_access / number_vanilla)*100 )
                    break
