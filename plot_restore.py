...
data = np.genfromtxt('Restore_pageCross_perPageCross.csv',delimiter=',',skip_header=0,usecols=(0,1))

program_names = ["bc" ,"bfs",  "sssp" ,"pr" , "pr_spmv","cc_sv" ,"tc"]
...
baseline_plot = ax.bar(x  , baseline, label='Baseline', width=0.4, color='#000000', edgecolor='black', linewidth=1,hatch ='///')
...
ax.set_ylabel('Restore from Table', fontproperties=prop, fontsize=12)
...
section_positions = [6]  # Example positions where you want to mark the sections
...
for position, label in zip(section_positions, section_labels):
    ...
    ax.text(position,95, label, transform=ax.transData, ha='right', va='top', fontsize=12)  # Add the section label
...
ax.axhline(y=60, color='grey', linestyle='--',alpha=0.5)
...
plt.tight_layout()
plt.savefig('Restore.pdf',format='pdf', dpi=300)
#plt.show()
