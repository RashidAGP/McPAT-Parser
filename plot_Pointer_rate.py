...
data = np.genfromtxt('Pointer_Chasing.csv',delimiter=',',skip_header=0,usecols=(0,1))
...
l0tlb_plot = ax.bar(x  , baseline, label='Pointer-chasing ratio', width=0.35, color='#b3b0b0', edgecolor='black', linewidth=1,hatch ='//')
...
ax.set_ylabel('Pointer-chasing ratio', fontproperties=prop, fontsize=10)
...
bars = ax.bar(range(len(program_names)), baseline, width=0.8, alpha=0, color='#01090b', edgecolor='black', linewidth=1.5)
...
section_positions = [5.5,13.5,16.35,20.5]  # Example positions where you want to mark the sections
...
for position, label in zip(section_positions, section_labels):
    ...
    ax.text(position,70, label, transform=ax.transData, ha='right', va='top', fontsize=12)  # Add the section label
...
ax.axhline(y=40, color='grey', linestyle='--',alpha=0.5)
...
plt.tight_layout()
plt.savefig('Pointer_Chasing.pdf',format='pdf', dpi=300)
plt.show()
