...
data = np.genfromtxt('power_results.csv',delimiter=',',skip_header=0,usecols=(0,1,2,3,4,5))
...
ax.set_ylabel('Power (Watt)', fontproperties=prop, fontsize=12)
...
for position, label in zip(section_positions, section_labels):
    if label == 'XSB':
        ...
        ax.text(position, 6, label, transform=ax.transData, ha='right', va='top', fontsize=11)  # Display vertically
    else:
        ax.axvline(x=position, color='white', linestyle='', linewidth=4)  # Mark the section with a vertical line
        ax.text(position, 6, label, transform=ax.transData, ha='right', va='top', fontsize=11)  # Display horizontally
...
ax.yaxis.set_major_formatter(ticker.FuncFormatter(lambda y, _: f'{y*1:.1f}'))
plt.tight_layout()
plt.savefig('power.pdf',format='pdf', dpi=300)
plt.show()
