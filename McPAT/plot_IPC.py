...
data = np.genfromtxt('IPC.csv',delimiter=',',skip_header=0,usecols=(0,1,2))

program_names = ["cactuBSSN", "exchange2", "gcc", "imagick", "lbm" ,"leela", "mcf", "nab", "omnetpp","perlbench","xalancbmk", "crono_apsp", "crono_cc", "crono_sssp","crono_tc", "crono_tsp", "XSB", "bfs","cc" ,"cc_sv","pr", "pr_spmv", "sssp", "tc","GMEAN"]
...
baseline_plot = ax.bar(x - 0.2 , baseline, label='Baseline', width=0.4, color='#000000', edgecolor='black', linewidth=1,hatch ='///')
l0tlb_plot = ax.bar(x + 0.2 , l0tlb, label='RXT', width=0.4, color='#33b24c', edgecolor='black', linewidth=1,hatch ='//')
...
ax.set_ylabel('IPC', fontproperties=prop, fontsize=12)
...
section_positions = [10.5, 15.5, 16.5,23.5]  # Example positions where you want to mark the sections
...
'''
for bar, value in zip(bars, l0tlb):
    height = bar.get_height()
    ax.text(bar.get_x() + bar.get_width() / 2, height + 1, f'{value:.2f}', ha='center', va='bottom', fontsize=8, color='black')
'''

section_positions = [5.5,13.5,16.25,20.5]  # Example positions where you want to mark the sections
...
for position, label in zip(section_positions, section_labels):
    ...
    ax.text(position,103, label, transform=ax.transData, ha='right', va='top', fontsize=11)  # Add the section label
'''
...
for position, label in zip(section_positions, section_labels):
    if label == 'XSB':
        ...
        ax.text(position, 102, label, transform=ax.transData, ha='right', va='top', fontsize=11)  # Display vertically
    else:
        ax.axvline(x=position, color='white', linestyle='', linewidth=4)  # Mark the section with a vertical line
        ax.text(position, 102, label, transform=ax.transData, ha='right', va='top', fontsize=11)  # Display horizontally



fig = plt.gcf()
fig.set_size_inches(12,2.4)
...
ax.axhline(y=103, color='grey', linestyle='--',alpha=0.5)
