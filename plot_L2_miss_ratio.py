...
data = np.genfromtxt('/home/rashid/extract_data/TLB_L2_miss_ratio.csv',delimiter=',',skip_header=0,usecols=(0,1))
...
program_names = ["cactuBSSN", "exchange2", "gcc", "imagick", "lbm" ,"leela", "mcf", "nab", "omnetpp","perlbench","xalancbmk", "crono_apsp", "crono_cc", "crono_sssp","crono_tc", "crono_tsp", "XSB", "bfs","cc" ,"cc_sv","pr", "pr_spmv", "sssp", "tc"]
...
l0tlb_plot = ax.bar(x  , baseline, label='T-Reg', width=0.35, color='#b3b0b0', edgecolor='black', linewidth=1,hatch ='//')
...
ax.set_ylabel('L2 TLB miss ratio', fontproperties=prop, fontsize=13)
...
section_positions = [7,13.5,16.35,20.5]  # Example positions where you want to mark the sections
...
plt.tight_layout()
plt.savefig('L2_TLB_miss_ratio.pdf',format='pdf', dpi=300, bbox_inches='tight')
plt.show()
