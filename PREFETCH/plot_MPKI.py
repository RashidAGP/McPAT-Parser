...
data = np.genfromtxt('TLB_MPKI.csv',delimiter=',',skip_header=0,usecols=(0,1))
...
program_names = [ "bc" ,"bfs",  "sssp" ,"pr" , "pr_spmv","cc","cc_sv" ,"tc", "GMEAN"]
...
l0tlb_plot = ax.bar(x  , baseline, label='L1 TLB MPKI', width=0.25, color='#b3b0b0', edgecolor='black', linewidth=1,hatch ='//')
...
ax.set_ylabel('TLB MPKI', fontproperties=prop, fontsize=14)
...
for bar, value in zip(bars, baseline):
    ...
    ax.text(bar.get_x() + bar.get_width() / 2, height + 1, f'{value:.2f}', ha='center', va='bottom', fontsize=12, color='black')
...
section_positions = [4]  # Example positions where you want to mark the sections
section_labels = ['GAPBS']  # Example labels for the sections


for position, label in zip(section_positions, section_labels):
    ax.axvline(x=position, color='white', linestyle='', linewidth=4)  # Mark the section with a vertical line
    ax.text(position,105, label, transform=ax.transData, ha='right', va='top', fontsize=16)  # Add the section label
...
#ax.axhline(y=20, color='grey', linestyle='--',alpha=0.8)
ax.axhline(y=40, color='grey', linestyle='--',alpha=0.3)
#ax.axhline(y=60, color='grey', linestyle='--',alpha=0.8)
ax.axhline(y=80, color='grey', linestyle='--',alpha=0.3)
#ax.axhline(y=100, color='grey', linestyle='--',alpha=0.8)

plt.xlim(-0.5, len(program_names) - 0.5)
# Apply custom font to program names on x-axis
#plt.xticks(x, programs, fontproperties=prop, fontsize=16)
plt.xticks(range(len(program_names)),program_names, fontproperties=prop, fontsize=12)
#plt.ylabel("Y-axis", fontproperties=prop)
plt.ylim(bottom=0,top=92)
...
plt.tight_layout()
plt.savefig('TLB_MPKI.pdf',format='pdf', bbox_inches='tight',dpi=300)
#plt.show()
