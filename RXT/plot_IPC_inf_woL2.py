...
data = np.genfromtxt('IPC_inf_woL2.csv',delimiter=',',skip_header=0,usecols=(0,1,2))
...
program_names = [ "bc" ,"bfs",  "sssp" ,"pr" , "pr_spmv","cc_sv" ,"tc", "GMEAN"]
...
for position, label in zip(section_positions, section_labels):
    ...
    ax.text(position,107, label, transform=ax.transData, ha='right', va='top', fontsize=12)  # Add the section label
...
ax.axhline(y=105, color='grey', linestyle='--',alpha=0.5)
...
plt.tight_layout()
plt.savefig('fig_IPC_inf_woL2.pdf',format='pdf',bbox_inches='tight', dpi=300)
#plt.show()
