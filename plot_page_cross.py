...
data = np.genfromtxt('PageCross.csv',delimiter=',',skip_header=0,usecols=(0,1))

program_names = ["cactuBSSN","lbm", "mcf", "exchange2", "nab", "leela", "imagick", "perlbench", "omnetpp", "xalancbmk", "gcc", "bc" ,"bfs",  "sssp" ,"pr" , "pr_spmv","cc_sv" ,"tc"]
...
l0tlb_plot = ax.bar(x  , baseline, label='Page-Cross ratio', width=0.25, color='#b3b0b0', edgecolor='black', linewidth=1,hatch ='//')
...
ax.set_ylabel('Page-cross ratio', fontproperties=prop, fontsize=8)
...
for position, label in zip(section_positions, section_labels):
    ...
    ax.text(position,80, label, transform=ax.transData, ha='right', va='top', fontsize=12)  # Add the section label
...
plt.tight_layout()
plt.savefig('PageCross.pdf',format='pdf', dpi=300)
#plt.show()
