...
data = np.genfromtxt('/home/rashid/extract_data/L1_TLB_MPKI.csv',delimiter=',',skip_header=0,usecols=(0,1))
...
ax.set_ylabel('L1 TLB MPKI', fontproperties=prop, fontsize=8)
...
for position, label in zip(section_positions, section_labels):
    ...
    ax.text(position,90, label, transform=ax.transData, ha='right', va='top', fontsize=12)  # Add the section label
...
ax.axhline(y=20, color='grey', linestyle='--',alpha=0.4)
ax.axhline(y=40, color='grey', linestyle='--',alpha=0.4)
ax.axhline(y=60, color='grey', linestyle='--',alpha=0.4)
ax.axhline(y=80, color='grey', linestyle='--',alpha=0.4)
...
plt.tight_layout()
plt.savefig('L1_TLB_MPKI.pdf',format='pdf', dpi=300)
plt.show()
