...
data = np.genfromtxt('reuse.csv',delimiter=',',skip_header=0,usecols=(0,1))
...
ax.set_ylabel('Reusibility of pointers', fontproperties=prop, fontsize=11)
...
for position, label in zip(section_positions, section_labels):
    ...
    ax.text(position,96, label, transform=ax.transData, ha='right', va='top', fontsize=15)  # Add the section label
...
plt.tight_layout()
plt.savefig('Reuse.pdf',format='pdf', dpi=300)
plt.show()
